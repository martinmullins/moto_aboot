//
// This file was generated by the Retargetable Decompiler
// Website: https://retdec.com
// Copyright (c) 2017 Retargetable Decompiler <info@retdec.com>
//

#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>

// ------------------------ Structures ------------------------

struct tm {
    int32_t e0;
    int32_t e1;
    int32_t e2;
    int32_t e3;
    int32_t e4;
    int32_t e5;
    int32_t e6;
    int32_t e7;
    int32_t e8;
    int32_t e9;
    char * e10;
};

// ------------------- Function Prototypes --------------------

int32_t copy_git_line(int32_t * a1, int32_t a2, int32_t a3);
int32_t getvar_aboot_git(int32_t a1, int32_t a2);
int32_t getvar_current_time(char * str, int32_t a2, int32_t a3);
int32_t getvar_emmc(void);
int32_t getvar_frp_state(char * a1);
int32_t getvar_hyp_git(int32_t a1, int32_t a2);
int32_t getvar_max_download_size(char * str, int32_t a2);
int32_t getvar_max_sparse_size(char * str, int32_t a2);
int32_t getvar_mot_sst(char * str, int32_t a2, int32_t a3);
int32_t getvar_ram(int32_t a1, int32_t a2);
int32_t getvar_reason(int32_t a1, int32_t a2, int32_t a3);
int32_t getvar_rpm_git(int32_t a1, int32_t a2);
int32_t getvar_sbl1_git(int32_t a1, int32_t a2);
int32_t getvar_tz_git(int32_t a1, int32_t a2);
char * getvar_uid(int32_t a1);
int32_t getvar_version(int32_t a1, int32_t a2);

// --------------------- Global Variables ---------------------

int32_t g6 = 0; // R4
int32_t g1 = -0x1a60cfc8; // 0x8f62bd74
int32_t g2 = 0; // 0x8f7112f4
int32_t g3 = 0; // 0x8f711464
int32_t g4 = 0; // 0x8f711500
int32_t g5 = 0; // 0x8f7125ec

// ------------------------ Functions -------------------------

// Address range: 0x8f62be40 - 0x8f62bf4f
int32_t getvar_frp_state(char * a1) {
    // 0x8f62be40
    int32_t v1;
    int32_t v2 = v1; // bp-148
    int32_t str = (int32_t)a1; // R4
    int32_t v3;
    char * v4 = get_partition_by_name((int32_t)"frp", 0, 0, g2, v3, 0, 0, v1, 0, 0, 0); // 0x8f62be60
    int32_t v5 = (int32_t)v4; // 0x8f62be60_12
    if (v4 != NULL) {
        char v6 = *(char *)(v5 + 33); // 0x8f62be6c
        char v7 = *(char *)(v5 + 32); // 0x8f62be70
        char v8 = *(char *)(v5 + 37); // 0x8f62be74
        char v9 = *(char *)(v5 + 34); // 0x8f62be7c
        int32_t v10 = 0x10000 * (int32_t)v9 | (int32_t)v7 | 256 * (int32_t)v6; // 0x8f62be80
        char v11 = *(char *)(v5 + 35); // 0x8f62be84
        char v12 = *(char *)(v5 + 36); // 0x8f62be8c
        char v13 = *(char *)(v5 + 38); // 0x8f62be90
        int32_t v14 = &v2; // 0x8f62bea0_0
        int32_t v15 = 512 * ((int32_t)v12 | 256 * (int32_t)v8 | 0x10000 * (int32_t)v13) | (0x1000000 * (int32_t)v11 | v10) / 0x800000; // 0x8f62beb0
        int32_t v16;
        int32_t v17;
        if (generic_mmc_read_logical(512 * v10, v15, 128, 0, v14, 0, 0, v1, 0, 0, 0, 0, 0, 0, 0, v17, v16, 0, 0, 0, 0, 0, 0) == 0) {
            // 0x8f62bec4
            if (v17 == 0x73189019) {
                int32_t v18 = is_factory_reset_protect_locked(0, v15, 0x73189019, 0x73189019, v14, 0, 0, v2); // 0x8f62bed8
                int32_t format = v18 != 0 & llvm_bswap_i32(v16) > 10 ? (int32_t)"protected (%d)" : (int32_t)"no protection (%d)";
                sprintf((char *)str, (char *)format);
                // branch -> 0x8f62bf18
                // 0x8f62bf30
                return 1;
            }
        }
    }
    // 0x8f62bf0c
    sprintf((char *)str, "no protection (err)");
    // branch -> 0x8f62bf18
    // 0x8f62bf30
    return 1;
}

// --------------- Statically Linked Functions ----------------

// void __stack_chk_fail(void);
// char * asctime(const struct tm * tp);
// struct tm * gmtime(const time_t * timer);
// void * memset(void * s, int c, size_t n);
// int snprintf(char * restrict s, size_t maxlen, const char * restrict format, ...);
// int sprintf(char * restrict s, const char * restrict format, ...);
// time_t time(time_t * timer);

// --------------- Instruction-Idiom Functions ----------------

// int32_t llvm_bswap_i32(int32_t a1);

// --------------------- Meta-Information ---------------------

// Detected compiler/packer: android-ndk-r8 (4.4.3 armv5te, armv7a(arm-linux-androideabi-gcc))
// Detected functions: 16
// Decompiler release: v2.2.1 (2016-09-07)
// Decompilation date: 2017-08-06 18:40:35
