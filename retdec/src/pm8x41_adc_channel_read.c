//
// This file was generated by the Retargetable Decompiler
// Website: https://retdec.com
// Copyright (c) 2017 Retargetable Decompiler <info@retdec.com>
//

#include <stdbool.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>

// ---------------- Integer Types Definitions -----------------

typedef int8_t int2_t;

// ------------------- Function Prototypes --------------------

int32_t adc_configure(int32_t * a1);
int32_t copyrect16(int32_t * a1, int32_t a2, int32_t a3, int32_t a4, int32_t a5, int32_t a6);
int32_t copyrect32(int32_t * a1, int32_t a2, int32_t a3, int32_t a4, int32_t a5, int32_t a6);
int32_t enable_irq(char * a1, int32_t a2, int32_t a3);
int32_t fillrect16(int32_t * a1, int32_t a2, int32_t a3, int32_t a4, int32_t a5);
int32_t fillrect24(int32_t * a1, int32_t a2, int32_t a3, uint32_t a4, int32_t a5);
int32_t fillrect32(int32_t * a1, int32_t a2, int32_t a3, int32_t a4, int32_t a5);
int32_t is_sufficient_pwr_avail(char * a1);
int32_t is_valid_supplier_id_95(int32_t a1, int32_t a2, int32_t a3);
int32_t oem_smb1359(int32_t a1, int32_t a2, int32_t a3);
int32_t oem_smb1359_usage(int32_t a1, int32_t a2, int32_t a3);
int32_t panel_init_96(int32_t * a1, int32_t a2, int32_t a3);
int32_t pm8x41_adc_channel_read(int32_t a1);
int32_t pm8x41_reg_read(uint32_t a1);
int32_t putpixel32(int32_t * a1, int32_t a2, int32_t a3, int32_t a4);
int32_t smb1359_is_hvdcp(int32_t a1);
int32_t smb1359_masked_write_reg(char * a1, int32_t a2, int32_t a3);
int32_t smb1359_power_source_detected(int32_t a1);
int32_t type_atoi_98(char * str, int32_t a2, int32_t a3);
int32_t update_panel_init_cmds_94(int32_t a1, int32_t a2);
int32_t update_panel_init_cmds_97(int32_t a1, int32_t a2);
char * usb_call_back(void);

// --------------------- Global Variables ---------------------

int32_t g27 = 0; // LR
int32_t g28 = 0; // R0
int32_t g29 = 0; // R1
int32_t g30 = 0; // R2
int32_t g31 = 0; // R3
int32_t g32 = 0; // R4
int32_t g33 = 0; // R5
int32_t g34 = 0; // R6
int32_t g35 = 0; // R7
int32_t g36 = 0; // R8
int32_t g1 = 0x2020003a; // 0x8f686ac4
int32_t g2 = 0x746e6300; // 0x8f6a4e20
int32_t g3 = 8; // 0x8f6aecc4
int32_t g4 = 8; // 0x8f6aed44
int32_t g5 = 1025; // 0x8f6aedcc
int32_t g7 = 7; // 0x8f6aede0
int32_t g8 = 720; // 0x8f6aef24
int32_t g9 = 1280; // 0x8f6aef28
int32_t g10 = 24; // 0x8f6aef30
int32_t g11 = 0; // 0x8f6aef44
int32_t g12 = 8; // 0x8f6aefe4
int32_t g13 = 8; // 0x8f6afc44
char * g14 = "\x01\x04"; // 0x8f6afce4
char g15 = 4; // 0x8f6afce5
int32_t g17 = 7; // 0x8f6afcf8
int32_t g18 = 8; // 0x8f6afd0c
int32_t g19 = 0x3100; // 0x8f6afddc
int16_t g20 = 49; // 0x8f6afde2
int32_t g21 = 0x3100; // 0x8f6afde8
int16_t g22 = 6; // 0x8f6afdee
int32_t g23 = 0x3100; // 0x8f6afdf4
int32_t g24 = 0; // 0x8f6e0b64
int32_t g25 = 0; // 0x8f6e0bd4
int32_t g26 = 0; // 0x8f7112f4
int32_t * g6 = &g4; // 0x8f6aeddc
int32_t * g16 = &g18; // 0x8f6afcf4

// ------------------------ Functions -------------------------

// Address range: 0x8f64635c - 0x8f6464eb
int32_t pm8x41_adc_channel_read(int32_t a1) {
    int32_t v1 = g32; // 0x8f64635c
    int32_t v2 = g33; // 0x8f64635c
    int32_t v3 = g34; // 0x8f64635c
    int32_t v4;
    int32_t v5 = &v4; // 0x8f646360_0
    int32_t v6 = g26;
    g30 = g20;
    int32_t v7; // 0x8f6464b420
    int32_t v8; // 0x8f6464b421
    int32_t v9; // 0x8f64639c
    int32_t v10; // R4
    int32_t v11;
    int32_t v12;
    int32_t v13;
    int32_t v14; // 0x8f646410
    int32_t v15; // 0x8f646498
    int32_t v16; // 0x8f6464a0
    int32_t v17; // 0x8f6464a8
    int32_t v18; // 0x8f6464ac
    int32_t result; // 0x8f6464c0
    char v19; // 0x8f6463a0
    int32_t v20; // 0x8f646424
    if ((int32_t)g20 == a1) {
        // 0x8f646394
        g32 = &g19;
        // branch -> 0x8f646398
        // 0x8f646398
        v9 = adc_configure((int32_t *)(int32_t)&g19);
        v19 = *(char *)(g32 + 11);
        g32 = v19;
        g34 = v9;
        switch (v19) {
            default: {
                // if_8f646430_0_true
                v10 = 0;
                v18 = 0;
                v17 = v9;
                v8 = v12;
                v7 = v13;
                // branch -> 0x8f6464a4
                break;
            }
            case 0: {
                // 0x8f6463b0
                g33 = 1;
                v4 = 0x3100;
                adc_configure(&v4);
                v4 = 0x3100;
                adc_configure(&v4);
                v14 = g34;
                v20 = 3 * 0x98968 / (int32_t)&v11 * (v14 - v5) + 0x1c9c38;
                v10 = v20;
                v18 = v20;
                v17 = v14;
                v8 = 0x1000000 * g33 / 0x1000000;
                v7 = 0x1000000 * g32 / 0x1000000;
                // branch -> 0x8f6464a4
                break;
            }
            case 1: {
                // 0x8f646438
                g33 = 0;
                v4 = 0x3100;
                adc_configure(&v4);
                v4 = 0x3100;
                adc_configure(&v4);
                v15 = g34;
                v16 = 0x1b7740 / (int32_t)&v11 * (v15 - v5);
                v10 = v16;
                v18 = v16;
                v17 = v15;
                v8 = 0x1000000 * g32 / 0x1000000;
                v7 = 0x1000000 * g33 / 0x1000000;
                // branch -> 0x8f6464a4
                break;
            }
        }
        // 0x8f6464a4
        print_log(2, (int32_t)"Result: Raw %u\tCalibrated:%u\n", v17, v18, v4, v7, v8, v6, 0, v1, v2, v3, g35);
        result = v10;
        g28 = result;
        if (v6 != g26) {
            // 0x8f6464cc
            __stack_chk_fail();
            // branch -> 0x8f6464d0
        }
        // 0x8f6464d0
        g32 = v1;
        g33 = v2;
        g34 = v3;
        return result;
    }
    // 0x8f646380
    int32_t v21; // 0x8f646398
    if ((int32_t)g22 == a1) {
        // if_8f646388_0_true
        g32 = &g21;
        v21 = &g21;
        // branch -> 0x8f646398
    } else {
        // if_8f64638c_0_true
        g32 = &g23;
        v21 = &g23;
        // branch -> 0x8f646398
    }
    // 0x8f646398
    v9 = adc_configure((int32_t *)v21);
    v19 = *(char *)(g32 + 11);
    g32 = v19;
    g34 = v9;
    switch (v19) {
        default: {
            // if_8f646430_0_true
            v10 = 0;
            v18 = 0;
            v17 = v9;
            v8 = v12;
            v7 = v13;
            // branch -> 0x8f6464a4
            break;
        }
        case 0: {
            // 0x8f6463b0
            g33 = 1;
            v4 = 0x3100;
            adc_configure(&v4);
            v4 = 0x3100;
            adc_configure(&v4);
            v14 = g34;
            v20 = 3 * 0x98968 / (int32_t)&v11 * (v14 - v5) + 0x1c9c38;
            v10 = v20;
            v18 = v20;
            v17 = v14;
            v8 = 0x1000000 * g33 / 0x1000000;
            v7 = 0x1000000 * g32 / 0x1000000;
            // branch -> 0x8f6464a4
            break;
        }
        case 1: {
            // 0x8f646438
            g33 = 0;
            v4 = 0x3100;
            adc_configure(&v4);
            v4 = 0x3100;
            adc_configure(&v4);
            v15 = g34;
            v16 = 0x1b7740 / (int32_t)&v11 * (v15 - v5);
            v10 = v16;
            v18 = v16;
            v17 = v15;
            v8 = 0x1000000 * g32 / 0x1000000;
            v7 = 0x1000000 * g33 / 0x1000000;
            // branch -> 0x8f6464a4
            break;
        }
    }
    // 0x8f6464a4
    print_log(2, (int32_t)"Result: Raw %u\tCalibrated:%u\n", v17, v18, v4, v7, v8, v6, 0, v1, v2, v3, g35);
    result = v10;
    g28 = result;
    if (v6 != g26) {
        // 0x8f6464cc
        __stack_chk_fail();
        // branch -> 0x8f6464d0
    }
    // 0x8f6464d0
    g32 = v1;
    g33 = v2;
    g34 = v3;
    return result;
}

// --------------- Statically Linked Functions ----------------

// void __stack_chk_fail(void);
// long int atol(const char * nptr);
// void * memset(void * s, int c, size_t n);
// int strcmp(const char * s1, const char * s2);
// int strncmp(const char * s1, const char * s2, size_t n);
// char * strncpy(char * restrict dest, const char * restrict src, size_t n);
// char * strrchr(char * s, int c);

// --------------------- Meta-Information ---------------------

// Detected compiler/packer: android-ndk-r8 (4.4.3 armv5te, armv7a(arm-linux-androideabi-gcc))
// Detected functions: 22
// Decompiler release: v2.2.1 (2016-09-07)
// Decompilation date: 2017-08-06 18:49:14
