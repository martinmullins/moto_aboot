//
// This file was generated by the Retargetable Decompiler
// Website: https://retdec.com
// Copyright (c) 2017 Retargetable Decompiler <info@retdec.com>
//

#include <ctype.h>
#include <stdbool.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

// ---------------- Integer Types Definitions -----------------

typedef int8_t int4_t;

// ------------------- Function Prototypes --------------------

int32_t fastboot_usb_udc_device_setup(int32_t a1, int32_t a2, int32_t a3);
int32_t font_get_y(int32_t a1, int32_t a2, int32_t a3);
void function_8f64b4dc(void);
void function_8f64b57c(char * a1, int32_t a2);
int32_t generic_mmc_init(char * a1, int32_t a2);
int32_t hab_command_parser(int32_t * a1, int32_t * a2, int32_t a3);
int32_t hab_health_check(int32_t a1, int32_t a2, int32_t a3);
int32_t hab_rehash(int32_t result, int32_t a2, char * a3);
int32_t hab_sahara_hash(char * a1, int32_t a2, int32_t a3);
int32_t is_bare_board(int32_t a1, int32_t a2);
int32_t keys_init(int32_t a1, int32_t a2, int32_t a3);
int32_t keys_post_event(uint32_t a1, int32_t a2);
int32_t led_off(int32_t a1, int32_t a2, int32_t a3);
int32_t mcs_ctx_init_rsa_sig(char a1, char a2, char a3, int32_t a4, uint32_t a5);
int32_t mcs_set_rsa_keys(char * a1, int32_t a2, int32_t a3, uint32_t a4, int32_t a5, int32_t a6, int32_t a7, int32_t a8, int32_t a9);
int32_t putpixel32(int32_t * a1, int32_t a2, int32_t a3, int32_t a4);
char * scm_halt_pmic_arbiter(int32_t a1);
int32_t scm_protect_keystore(int32_t a1, int32_t a2, int32_t a3);
int32_t set_download_mode(int32_t a1);
int32_t target_baseband_detect(int32_t * a1);
int32_t target_display_init(int32_t a1, int32_t a2);
int32_t target_get_reset_status(char * a1, int32_t a2, int32_t a3);
int32_t target_load_ssd_keystore(int32_t a1, int32_t a2, int32_t a3, int32_t a4, int32_t a5, int32_t a6, int32_t a7, int32_t a8, int32_t a9, int32_t a10);
int32_t target_mmc_do_write_protect(int32_t a1, int32_t a2, int32_t a3);
int32_t target_panel_clock(int32_t a1, int32_t a2, int32_t a3);
int32_t target_product_name(void);
int32_t target_set_raminfo(void);
int32_t update_meminfo_to_devtree(int32_t a1, int32_t a2);
int32_t update_mmi_panel_find_node(int32_t a1, int32_t a2, char * a3, int32_t a4);

// --------------------- Global Variables ---------------------

int32_t g27 = 0; // LR
int32_t g28 = 0; // R0
int32_t g29 = 0; // R1
int32_t g30 = 0; // R10
int32_t g31 = 0; // R11
int32_t g32 = 0; // R2
int32_t g33 = 0; // R4
int32_t g34 = 0; // R5
int32_t g35 = 0; // R6
int32_t g36 = 0; // R7
int32_t g37 = 0; // R8
int32_t g38 = 0; // R9
char * g1[15] = {
    "unknown",
    "Samsung",
    "Qimonda",
    "Elpida",
    "Etron",
    "Nanya",
    "Hynix",
    "Mosel",
    "Winbond",
    "ESMT",
    "unknown",
    "Spansion",
    "SST",
    "ZMOS",
    "Intel"
}; // 0x8f6817d4
char * g2[19] = {
    "S4 SDRAM",
    "S2 SDRAM",
    "N NVM",
    "S8 SDRAM",
    "64Mb",
    "128Mb",
    "256Mb",
    "512Mb",
    "1Gb",
    "2Gb",
    "4Gb",
    "8Gb",
    "16Gb",
    "32Gb",
    "reserved",
    "reserved",
    "reserved",
    "12Gb",
    "6Gb"
}; // 0x8f681a84
char * g3[15] = {
    "64Mb",
    "128Mb",
    "256Mb",
    "512Mb",
    "1Gb",
    "2Gb",
    "4Gb",
    "8Gb",
    "16Gb",
    "32Gb",
    "reserved",
    "reserved",
    "reserved",
    "12Gb",
    "6Gb"
}; // 0x8f681a94
int32_t g4 = 0x65440030; // 0x8f69c66f
int32_t g5 = 0xf0e0d; // 0x8f6a6b06
int32_t g6 = 0; // 0x8f6b7954
int32_t g7 = 0; // 0x8f6b8bdc
int32_t g8 = 0; // 0x8f6b8be0
int32_t g9 = 0; // 0x8f6b8be8
int32_t g10 = 0; // 0x8f6b8bec
char * g11; // 0x8f6c0018
char * g12; // 0x8f6e0af4
char * g13; // 0x8f6e0b34
int32_t g14 = 0; // 0x8f6e0be4
char * g15; // 0x8f6e0c48
char * g16; // 0x8f7112f4
char * g17; // 0x8f711464
int32_t g18 = 0; // 0x8f7125d0
char * g19; // 0x8f7125ec
int32_t g20 = 0; // 0x8f7129d8
int32_t g21 = 0; // 0x8f7129e8
int32_t g22 = 0; // 0x8f7129f8
int32_t g23 = 0; // 0x8f712a08
int32_t g24 = 0; // 0x8f712a18
int32_t g25 = 0; // 0x8f712e28
int32_t g26 = 0; // 0x8f712f2c

// ------------------------ Functions -------------------------

// Address range: 0x8f66d428 - 0x8f66d587
int32_t hab_sahara_hash(char * a1, int32_t a2, int32_t a3) {
    int32_t v1 = (int32_t)a1;
    int32_t v2 = g34; // 0x8f66d428
    int32_t v3 = g35; // 0x8f66d428
    int32_t v4;
    int32_t v5 = &v4; // 0x8f66d434_0
    char * v6 = g16;
    char v7 = *a1; // 0x8f66d440
    int32_t v8 = v7; // 0x8f66d440
    if (a3 == 0) {
        // 0x8f66d560
        if (v6 != g16) {
            // 0x8f66d570
            __stack_chk_fail();
            // branch -> 0x8f66d574
        }
        // 0x8f66d574
        g34 = v2;
        g35 = v3;
        return 57;
    }
    int32_t v9 = a2 == 98; // 0x8f66d458
    g35 = v9;
    int32_t result = 57;
    int32_t v10;
    int32_t v11; // 0x8f66d52c
    int32_t v12; // 0x8f66d548
    int32_t v13; // 0x8f66d46c_0
    int32_t v14; // 0x8f66d504_0
    int32_t v15; // LR
    int32_t v16; // 0x8f66d4d0
    int32_t v17; // 0x8f66d548
    int32_t v18; // R12
    int32_t v19; // R3
    int32_t v20; // R7
    int32_t v21;
    int32_t v22;
    int32_t v23; // 0x8f66d52c6
    int32_t v24;
    int32_t v25; // 0x8f66d4d8
    int32_t v26; // 0x8f66d518
    int32_t v27; // 0x8f66d4d8
    int32_t v28; // 0x8f66d488
    char v29; // 0x8f66d480
    char v30; // 0x8f66d490
    char v31; // 0x8f66d4b4
    int32_t v32; // 0x8f66d488
    int32_t v33; // 0x8f66d4a4
    int32_t v34; // 0x8f66d4ac
    int32_t v35; // 0x8f66d4bc
    int32_t v36; // 0x8f66d4c4
    int32_t v37; // 0x8f66d4cc
    int32_t v38; // 0x8f66d494
    switch (a2) {
        case 98: {
            // 0x8f66d468
            v13 = &v21;
            v15 = v13;
            v26 = 0;
            if (v7 != 0) {
                int32_t v39 = v13; // R1
                v19 = 0;
                v16 = v13;
                int32_t v40 = v8; // 0x8f66d484
                v28 = v1 + 1;
                // branch -> 0x8f66d480
                while (true) {
                    // 0x8f66d480
                    v29 = *(char *)(v28 + 1);
                    v32 = (int32_t)*(char *)v28;
                    g34 = v32;
                    v30 = *(char *)(v28 + 4);
                    v38 = (v40 - 1) % 0x10000;
                    v18 = v38;
                    v33 = (int32_t)*(char *)(v28 + 3);
                    g34 = v33;
                    v34 = (int32_t)*(char *)(v28 + 2);
                    g34 = v34;
                    v20 = 256 * v34 | v33 | 0x1000000 * v32 | 0x10000 * (int32_t)v29;
                    v31 = *(char *)(v28 + 5);
                    v35 = 0x10000 * (int32_t)v31 | 0x1000000 * (int32_t)v30;
                    g34 = v35;
                    v36 = v35 | (int32_t)*(char *)(v28 + 7);
                    g34 = v36;
                    v37 = 256 * (int32_t)*(char *)(v28 + 6) | v36;
                    g34 = v37;
                    *(int32_t *)v16 = v37;
                    *(int32_t *)(v16 + 4) = v20;
                    v19 += g34;
                    v25 = v39;
                    v27 = v25 + 12;
                    g34 = v27;
                    if (v38 == 0) {
                        // 0x8f66d4f0
                        *(int32_t *)(v25 + 8) = v18;
                        v26 = v19;
                        // branch -> 0x8f66d4f0
                        // 0x8f66d4f0
                        v14 = &v24;
                        v24 = 0x208d0024;
                        v10 = g35 == 0 ? 20 : 32;
                        if (v26 == 0) {
                            // if_8f66d530_0_true
                            v24 = -0x5e72ffdc;
                            v23 = v22;
                            v12 = -0x5e72ffdc;
                            // branch -> after_if_8f66d530_0
                        } else {
                            // after_if_8f66d524_0.thread
                            v23 = v5;
                            v12 = 0x208d0024;
                            // branch -> after_if_8f66d530_0
                        }
                        // after_if_8f66d530_0
                        v11 = v26 == 0 ? a3 : v23;
                        v4 = v10;
                        v17 = hab_sahara_operation(v14, v10, a3, 0, v10, a3, 0, v12, v26, v15, v10, v11);
                        g34 = v14;
                        result = 57;
                        if (v17 == 240) {
                            // 0x8f66d558
                            hab_su_type(240, v10);
                            result = v14;
                            // branch -> 0x8f66d560
                        }
                        // 0x8f66d560
                        if (v6 != g16) {
                            // 0x8f66d570
                            __stack_chk_fail();
                            // branch -> 0x8f66d574
                        }
                        // 0x8f66d574
                        g34 = v2;
                        g35 = v3;
                        return result;
                    }
                  lab_after_if_8f66d4e0_0_dec_label_pc_8f66d480_crit_edge:
                    // after_if_8f66d4e0_0.dec_label_pc_8f66d480_crit_edge
                    *(int32_t *)(v25 + 8) = v27;
                    int32_t v41 = g34; // 0x8f66d4e8
                    v39 = v41;
                    v16 = v41;
                    v40 = v18;
                    v28 += 8;
                    // branch -> 0x8f66d480
                }
            }
            // 0x8f66d4f0
            v14 = &v24;
            v24 = 0x208d0024;
            v10 = v9 == 0 ? 20 : 32;
            if (v26 == 0) {
                // if_8f66d530_0_true
                v24 = -0x5e72ffdc;
                v23 = v22;
                v12 = -0x5e72ffdc;
                // branch -> after_if_8f66d530_0
            } else {
                // after_if_8f66d524_0.thread
                v23 = v5;
                v12 = 0x208d0024;
                // branch -> after_if_8f66d530_0
            }
            // after_if_8f66d530_0
            v11 = v26 == 0 ? a3 : v23;
            v4 = v10;
            v17 = hab_sahara_operation(v14, v10, a3, 0, v10, a3, 0, v12, v26, v15, v10, v11);
            g34 = v14;
            if (v17 == 240) {
                // 0x8f66d558
                hab_su_type(240, v10);
                result = v14;
                // branch -> 0x8f66d560
            } else {
                result = 57;
            }
            break;
        }
        case 65: {
            // 0x8f66d468
            v13 = &v21;
            v15 = v13;
            v26 = 0;
            if (v7 != 0) {
                // 0x8f66d474
                v19 = 0;
                v16 = v13;
                v28 = v1 + 1;
                // branch -> 0x8f66d480
                while (true) {
                    // 0x8f66d480
                    v29 = *(char *)(v28 + 1);
                    v32 = (int32_t)*(char *)v28;
                    g34 = v32;
                    v30 = *(char *)(v28 + 4);
                    v38 = (v8 - 1) % 0x10000;
                    v18 = v38;
                    v33 = (int32_t)*(char *)(v28 + 3);
                    g34 = v33;
                    v34 = (int32_t)*(char *)(v28 + 2);
                    g34 = v34;
                    v20 = 256 * v34 | v33 | 0x1000000 * v32 | 0x10000 * (int32_t)v29;
                    v31 = *(char *)(v28 + 5);
                    v35 = 0x10000 * (int32_t)v31 | 0x1000000 * (int32_t)v30;
                    g34 = v35;
                    v36 = v35 | (int32_t)*(char *)(v28 + 7);
                    g34 = v36;
                    v37 = 256 * (int32_t)*(char *)(v28 + 6) | v36;
                    g34 = v37;
                    *(int32_t *)v16 = v37;
                    *(int32_t *)(v16 + 4) = v20;
                    v19 += g34;
                    v25 = v13;
                    v27 = v25 + 12;
                    g34 = v27;
                    if (v38 != 0) {
                        goto lab_after_if_8f66d4e0_0_dec_label_pc_8f66d480_crit_edge;
                    }
                    // 0x8f66d4f0
                    *(int32_t *)(v25 + 8) = v18;
                    // branch -> 0x8f66d4f0
                }
            }
            // 0x8f66d4f0
            v14 = &v24;
            v24 = 0x208d0024;
            v10 = v9 == 0 ? 20 : 32;
            if (v26 == 0) {
                // if_8f66d530_0_true
                v24 = -0x5e72ffdc;
                v23 = v22;
                v12 = -0x5e72ffdc;
                // branch -> after_if_8f66d530_0
            } else {
                // after_if_8f66d524_0.thread
                v23 = v5;
                v12 = 0x208d0024;
                // branch -> after_if_8f66d530_0
            }
            // after_if_8f66d530_0
            v11 = v26 == 0 ? a3 : v23;
            v4 = v10;
            v17 = hab_sahara_operation(v14, v10, a3, 0, v10, a3, 0, v12, v26, v15, v10, v11);
            g34 = v14;
            if (v17 == 240) {
                // 0x8f66d558
                hab_su_type(240, v10);
                result = v14;
                // branch -> 0x8f66d560
            } else {
                result = 57;
            }
            break;
        }
    }
    // 0x8f66d560
    if (v6 != g16) {
        // 0x8f66d570
        __stack_chk_fail();
        // branch -> 0x8f66d574
    }
    // 0x8f66d574
    g34 = v2;
    g35 = v3;
    return result;
}

// --------------- Statically Linked Functions ----------------

// void __stack_chk_fail(void);
// void free(void * ptr);
// void * malloc(size_t size);
// void * memcpy(void * restrict dest, const void * restrict src, size_t n);
// void * memset(void * s, int c, size_t n);
// int snprintf(char * restrict s, size_t maxlen, const char * restrict format, ...);
// int sprintf(char * restrict s, const char * restrict format, ...);
// int strcmp(const char * s1, const char * s2);
// int tolower(int c);

// --------------------- Meta-Information ---------------------

// Detected compiler/packer: android-ndk-r8 (4.4.3 armv5te, armv7a(arm-linux-androideabi-gcc))
// Detected functions: 29
// Decompiler release: v2.2.1 (2016-09-07)
// Decompilation date: 2017-08-06 19:20:53
