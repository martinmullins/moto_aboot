//
// This file was generated by the Retargetable Decompiler
// Website: https://retdec.com
// Copyright (c) 2017 Retargetable Decompiler <info@retdec.com>
//

#include <ctype.h>
#include <stdbool.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

// ------------------- Function Prototypes --------------------

int32_t arch_early_init(void);
int32_t boot_handler(int32_t a1, int32_t a2, int32_t a3, int32_t a4, int32_t a5);
int32_t boot_linux(int32_t (*a1)(int32_t, int32_t, int32_t), int32_t a2, int32_t a3, int32_t a4, int32_t a5);
int32_t disable_charger_gone_irq(int32_t a1, int32_t a2, int32_t a3);
int32_t enable_charger_gone_irq(int32_t a1, int32_t a2);
void function_8f64b730(void);
void function_8f64b7f0(char * str, char * delim, int32_t a3);
int32_t hab_policy_init(void);
int32_t init_barcodes(int32_t a1, int32_t a2, int32_t a3);
int32_t init_panels(void);
int32_t is_like_hab_signature(char * a1, int32_t a2, int32_t a3);
int32_t is_warranty_void(int32_t a1);
int32_t load_and_validate_logo(void);
int32_t LZ4_decompress_safe(char * a1, char * a2, int32_t a3, int32_t a4, int32_t a5);
int32_t LZ4_decompress_safe_withPrefix64k(char * a1, char * a2, int32_t a3, int32_t a4, int32_t a5);
int32_t mcs_habp_rsa_crt_core(int32_t a1, int32_t a2, int32_t a3, int32_t a4, int32_t a5, int32_t a6, int32_t a7);
int32_t mcs_habp_rsa_crt_med(int32_t a1, int32_t a2, int32_t a3);
int32_t mipi_mot_get_manufacture_id(int32_t * a1, int32_t a2, int32_t a3);
int32_t mipi_mot_get_supplier_id(int32_t * a1, int32_t a2, int32_t a3);
char * mmc_sd_set_bus_width(char * a1, int32_t * a2, int32_t a3);
char * mmc_sd_set_hs(char * a1);
int32_t move_to_verified(int32_t a1, int32_t a2, int32_t a3);
int32_t qup_i2c_sec_init(int32_t a1, int32_t a2, int32_t a3, int32_t a4, int32_t a5);
int32_t qup_i2c_xfer(int32_t a1);
int32_t radio_from_utag(int32_t a1, int32_t a2, int32_t a3);
char * restore_secure_cfg(int32_t a1, int32_t a2);
int32_t scm_init(int32_t a1);
int32_t screen_manager_init(int32_t a1, int32_t a2, int32_t a3, int32_t a4, int32_t a5);
int32_t show(char * a1, int32_t a2, int32_t a3);
int32_t target_hwrev(void);

// --------------------- Global Variables ---------------------

int32_t g45 = 0; // LR
int32_t g46 = 0; // R0
int32_t g47 = 0; // R1
int32_t g48 = 0; // R11
int32_t g49 = 0; // R12
int32_t g50 = 0; // R2
int32_t g51 = 0; // R3
int32_t g52 = 0; // R4
int32_t g53 = 0; // R5
int32_t g54 = 0; // R6
int32_t g55 = 0; // R7
int32_t g56 = 0; // R8
int32_t g57 = 0; // R9
int32_t g1 = -0x15fffffa; // 0x8f600000
int32_t g2 = -0x1a60cef8; // 0x8f612228
int32_t g3 = -0x1a60ceec; // 0x8f6383f0
int32_t g4 = -0x16d2bfc9; // 0x8f6387c0
int32_t g5 = -0x16d2bc09; // 0x8f638ab0
int32_t g6 = -0x1a60ff98; // 0x8f638b94
int32_t g7 = -0x16d2bfc9; // 0x8f64587c
char * g8 = "\xff\xff\xff\xff\x6c\x6f\x61\x64\x5f\x61\x6e\x64\x5f\x76\x61\x6c\x69\x64\x61\x74\x65\x5f\x6b\x65\x72\x6e\x65\x6c\x5f\x69\x6d\x61\x67\x65"; // 0x8f6826be
int32_t g9 = 0x65440030; // 0x8f69c66f
int32_t g10 = 0; // 0x8f6a3f38
int32_t g11 = 1; // 0x8f6ac0f0
int32_t g12 = -0x709c73f8; // 0x8f6ac374
int32_t g13 = 0xffff; // 0x8f6acae4
int32_t g14 = 4; // 0x8f6acaf4
int32_t g15 = 0xffff; // 0x8f6acb2c
int32_t g16 = 4; // 0x8f6acb84
int32_t g17 = 0; // 0x8f6b0048
int32_t g18 = 0; // 0x8f6b004c
int32_t g19 = 0; // 0x8f6b0074
int32_t g20 = 0; // 0x8f6b0078
int32_t g21 = 0; // 0x8f6b015c
int32_t g22 = 0; // 0x8f6b1608
int32_t g23 = 0; // 0x8f6b7cc0
int32_t g24 = 0; // 0x8f6c0000
int32_t g25 = 0; // 0x8f6c02aa
int32_t g26 = 0; // 0x8f6c02ba
int32_t g27 = 0; // 0x8f6c02ca
int32_t g28 = 0; // 0x8f6c02e0
int32_t g29 = 0; // 0x8f6e082c
char * g30; // 0x8f6e0849
int32_t g31 = 0; // 0x8f6e0854
int32_t g32 = 0; // 0x8f6e0858
int32_t g33 = 0; // 0x8f6e085c
int32_t g34 = 0; // 0x8f6e0878
int32_t g35 = 0; // 0x8f6e0894
int32_t g36 = 0; // 0x8f6e08b0
int32_t g37 = 0; // 0x8f6e08b4
char ** g38 = NULL; // 0x8f6e0bf8
int32_t g39 = 0; // 0x8f710c30
int32_t g40 = 0; // 0x8f710c34
int16_t * g41 = NULL; // 0x8f7112f4
int32_t g42 = 0; // 0x8f7114b8
int32_t g43 = 0; // 0x8f71302c
int32_t g44 = 0; // 0x8f713044

// ------------------------ Functions -------------------------

// Address range: 0x8f61d428 - 0x8f61d5f7
int32_t init_barcodes(int32_t a1, int32_t a2, int32_t a3) {
    int32_t v1 = g52; // 0x8f61d428
    int16_t * v2 = g41;
    int32_t v3 = (int32_t)v2;
    int32_t v4;
    int32_t v5 = __utag_gets_constprop_4_4("imei", (int32_t)&g27, 19, v3, a1, v3, a3, v1, g53, g45, v4, 0, 0, 0); // 0x8f61d444
    if (v5 == 0) {
        // 0x8f61d468
        strlcpy((int32_t)&g27, (int32_t)"000000000000000", 19, v3, a1);
        char * v6 = __utag_puts_constprop_5((int32_t)"imei", (int32_t)&g27, 19, v3, a1, v3, a3, v1); // 0x8f61d480
        g52 = v6 == NULL ? (int32_t)v6 : 1;
        if (v6 == NULL) {
            // 0x8f61d490
            print_log(1, (int32_t)"Failed to update default IMEI\n", 19, v3, a1, v3, a3, v1, g53, g45, v4, 0, 0);
            // branch -> 0x8f61d49c
        }
    } else {
        // 0x8f61d450
        g52 = 0;
        print_log(1, (int32_t)"IMEI/MEID: %s\n", (int32_t)&g27, v3, a1, v3, a3, v1, g53, g45, v4, 0, 0);
        // branch -> 0x8f61d49c
    }
    int32_t v7 = __utag_gets_constprop_4_4("sku", (int32_t)&g25, 16, v3, a1, v3, a3, v1, g53, g45, v4, 0, 0, 0); // 0x8f61d4a8
    int32_t v8; // 0x8f61d550
    int32_t v9; // 0x8f61d550
    int32_t v10; // 0x8f61d508
    int32_t v11; // 0x8f61d520
    char * v12; // 0x8f61d540
    int32_t v13; // 0x8f61d550
    int32_t v14; // 0x8f61d568
    int32_t result; // 0x8f61d598
    int32_t v15; // R2
    int32_t result2; // 0x8f61d5b4_2
    int32_t v16; // 0x8f61d5b4_23
    int32_t v17;
    int16_t * v18;
    if (v7 == 0) {
        // 0x8f61d4c8
        strlcpy((int32_t)&g25, (int32_t)"000000000000000", 16, v3, a1);
        char * v19 = __utag_puts_constprop_5((int32_t)"sku", (int32_t)&g25, 16, v3, a1, v3, a3, v1); // 0x8f61d4e0
        if (v19 == NULL) {
            // 0x8f61d4f0
            print_log(1, (int32_t)"Failed to update default SKU\n", 16, v3, a1, v3, a3, v1, g53, g45, v4, 0, 0);
            // branch -> 0x8f61d4fc
        } else {
            // if_8f61d4e8_0_true
            g52 = 1;
            // branch -> 0x8f61d4fc
        }
        // 0x8f61d4fc
        v10 = __utag_gets_constprop_4_4("date", (int32_t)&g26, 16, v3, a1, v3, a3, v1, g53, g45, v4, 0, 0, 0);
        if (v10 != 0) {
            // 0x8f61d514
            v11 = print_log(1, (int32_t)"DATE: %s\n", (int32_t)&g26, v3, a1, v3, a3, v1, g53, g45, v4, 0, 0);
            v17 = v11;
            // branch -> 0x8f61d56c
            // 0x8f61d56c
            g46 = v17;
            v16 = v17;
            if (g52 != 0) {
                // 0x8f61d54c
                v9 = &g26;
                v13 = __utags_flush(0, (int32_t)"DATE: %s\n", v9, v3, a1, v3, a3, v1);
                g46 = v13;
                if (v13 != 0) {
                    // 0x8f61d578
                    v18 = g41;
                    if (v2 != v18) {
                        // 0x8f61d5ac
                        __stack_chk_fail();
                        // branch -> 0x8f61d5b0
                        // 0x8f61d5b0
                        g52 = v1;
                        return g46;
                    }
                    // 0x8f61d588
                    g52 = v1;
                    result = print_log(1, (int32_t)"Failed to flash barcodes back\n", v3, (int32_t)v18, v1, g53, g45, v4, 0, 0, 0, 0, 0);
                    g46 = result;
                    return result;
                }
                v16 = 0;
            }
            // 0x8f61d59c
            if (v2 != g41) {
                // 0x8f61d5ac
                __stack_chk_fail();
                result2 = g46;
                // branch -> 0x8f61d5b0
            } else {
                result2 = v16;
            }
            // 0x8f61d5b0
            g52 = v1;
            return result2;
        }
        // 0x8f61d528
        v15 = 16;
        strlcpy((int32_t)&g26, (int32_t)"01-01-1970", 16, v3, a1);
        v12 = __utag_puts_constprop_5((int32_t)"date", (int32_t)&g26, v15, v3, a1, v3, a3, v1);
        if (v12 == NULL) {
            // 0x8f61d560
            v14 = print_log(1, (int32_t)"Failed to update default DATE\n", v15, v3, a1, v3, a3, v1, g53, g45, v4, 0, 0);
            v17 = v14;
            // branch -> 0x8f61d56c
            // 0x8f61d56c
            g46 = v17;
            if (g52 != 0) {
                // 0x8f61d54c
                v13 = __utags_flush(0, (int32_t)"Failed to update default DATE\n", v15, v3, a1, v3, a3, v1);
                g46 = v13;
                if (v13 != 0) {
                    // 0x8f61d578
                    v18 = g41;
                    if (v2 != v18) {
                        // 0x8f61d5ac
                        __stack_chk_fail();
                        // branch -> 0x8f61d5b0
                        // 0x8f61d5b0
                        g52 = v1;
                        return g46;
                    }
                    // 0x8f61d588
                    g52 = v1;
                    result = print_log(1, (int32_t)"Failed to flash barcodes back\n", v3, (int32_t)v18, v1, g53, g45, v4, 0, 0, 0, 0, 0);
                    g46 = result;
                    return result;
                }
                v16 = 0;
            } else {
                v16 = v17;
            }
        } else {
            // 0x8f61d54c
            v8 = &g26;
            v13 = __utags_flush(0, v8, v15, v3, a1, v3, a3, v1);
            g46 = v13;
            if (v13 != 0) {
                // 0x8f61d578
                v18 = g41;
                if (v2 != v18) {
                    // 0x8f61d5ac
                    __stack_chk_fail();
                    // branch -> 0x8f61d5b0
                    // 0x8f61d5b0
                    g52 = v1;
                    return g46;
                }
                // 0x8f61d588
                g52 = v1;
                result = print_log(1, (int32_t)"Failed to flash barcodes back\n", v3, (int32_t)v18, v1, g53, g45, v4, 0, 0, 0, 0, 0);
                g46 = result;
                return result;
            }
            v16 = 0;
        }
        // 0x8f61d59c
        if (v2 != g41) {
            // 0x8f61d5ac
            __stack_chk_fail();
            result2 = g46;
            // branch -> 0x8f61d5b0
        } else {
            result2 = v16;
        }
        // 0x8f61d5b0
        g52 = v1;
        return result2;
    }
    // 0x8f61d4b4
    print_log(1, (int32_t)"SKU: %s\n", (int32_t)&g25, v3, a1, v3, a3, v1, g53, g45, v4, 0, 0);
    // branch -> 0x8f61d4fc
    // 0x8f61d4fc
    v10 = __utag_gets_constprop_4_4("date", (int32_t)&g26, 16, v3, a1, v3, a3, v1, g53, g45, v4, 0, 0, 0);
    if (v10 != 0) {
        // 0x8f61d514
        v11 = print_log(1, (int32_t)"DATE: %s\n", (int32_t)&g26, v3, a1, v3, a3, v1, g53, g45, v4, 0, 0);
        v17 = v11;
        // branch -> 0x8f61d56c
        // 0x8f61d56c
        g46 = v17;
        if (g52 == 0) {
            // 0x8f61d59c
            if (v2 != g41) {
                // 0x8f61d5ac
                __stack_chk_fail();
                result2 = g46;
                // branch -> 0x8f61d5b0
            } else {
                result2 = v17;
            }
            // 0x8f61d5b0
            g52 = v1;
            return result2;
        }
        // 0x8f61d54c
        v9 = &g26;
        v13 = __utags_flush(0, (int32_t)"DATE: %s\n", v9, v3, a1, v3, a3, v1);
        g46 = v13;
        if (v13 == 0) {
            // 0x8f61d59c
            if (v2 != g41) {
                // 0x8f61d5ac
                __stack_chk_fail();
                result2 = g46;
                // branch -> 0x8f61d5b0
            } else {
                result2 = 0;
            }
            // 0x8f61d5b0
            g52 = v1;
            return result2;
        }
        // 0x8f61d578
        v18 = g41;
        if (v2 != v18) {
            // 0x8f61d5ac
            __stack_chk_fail();
            // branch -> 0x8f61d5b0
            // 0x8f61d5b0
            g52 = v1;
            return g46;
        }
        // 0x8f61d588
        g52 = v1;
        result = print_log(1, (int32_t)"Failed to flash barcodes back\n", v3, (int32_t)v18, v1, g53, g45, v4, 0, 0, 0, 0, 0);
        g46 = result;
        return result;
    }
    // 0x8f61d528
    v15 = 16;
    strlcpy((int32_t)&g26, (int32_t)"01-01-1970", 16, v3, a1);
    v12 = __utag_puts_constprop_5((int32_t)"date", (int32_t)&g26, v15, v3, a1, v3, a3, v1);
    if (v12 == NULL) {
        // 0x8f61d560
        v14 = print_log(1, (int32_t)"Failed to update default DATE\n", v15, v3, a1, v3, a3, v1, g53, g45, v4, 0, 0);
        v17 = v14;
        // branch -> 0x8f61d56c
        // 0x8f61d56c
        g46 = v17;
        if (g52 != 0) {
            // 0x8f61d54c
            v13 = __utags_flush(0, (int32_t)"Failed to update default DATE\n", v15, v3, a1, v3, a3, v1);
            g46 = v13;
            if (v13 != 0) {
                // 0x8f61d578
                v18 = g41;
                if (v2 != v18) {
                    // 0x8f61d5ac
                    __stack_chk_fail();
                    // branch -> 0x8f61d5b0
                    // 0x8f61d5b0
                    g52 = v1;
                    return g46;
                }
                // 0x8f61d588
                g52 = v1;
                result = print_log(1, (int32_t)"Failed to flash barcodes back\n", v3, (int32_t)v18, v1, g53, g45, v4, 0, 0, 0, 0, 0);
                g46 = result;
                return result;
            }
            v16 = 0;
        } else {
            v16 = v17;
        }
    } else {
        // 0x8f61d54c
        v8 = &g26;
        v13 = __utags_flush(0, v8, v15, v3, a1, v3, a3, v1);
        g46 = v13;
        if (v13 != 0) {
            // 0x8f61d578
            v18 = g41;
            if (v2 != v18) {
                // 0x8f61d5ac
                __stack_chk_fail();
                // branch -> 0x8f61d5b0
                // 0x8f61d5b0
                g52 = v1;
                return g46;
            }
            // 0x8f61d588
            g52 = v1;
            result = print_log(1, (int32_t)"Failed to flash barcodes back\n", v3, (int32_t)v18, v1, g53, g45, v4, 0, 0, 0, 0, 0);
            g46 = result;
            return result;
        }
        v16 = 0;
    }
    // 0x8f61d59c
    if (v2 != g41) {
        // 0x8f61d5ac
        __stack_chk_fail();
        result2 = g46;
        // branch -> 0x8f61d5b0
    } else {
        result2 = v16;
    }
    // 0x8f61d5b0
    g52 = v1;
    return result2;
}

// --------------- Statically Linked Functions ----------------

// void __stack_chk_fail(void);
// long int atol(const char * nptr);
// void free(void * ptr);
// int isdigit(int c);
// int isxdigit(int c);
// int memcmp(const void * s1, const void * s2, size_t n);
// void * memcpy(void * restrict dest, const void * restrict src, size_t n);
// void * memset(void * s, int c, size_t n);
// int snprintf(char * restrict s, size_t maxlen, const char * restrict format, ...);
// int strcmp(const char * s1, const char * s2);
// size_t strlen(const char * s);
// int strncmp(const char * s1, const char * s2, size_t n);
// char * strtok_r(char * restrict s, const char * restrict delim, char ** restrict save_ptr);
// int tolower(int c);

// --------------------- Meta-Information ---------------------

// Detected compiler/packer: android-ndk-r8 (4.4.3 armv5te, armv7a(arm-linux-androideabi-gcc))
// Detected functions: 30
// Decompiler release: v2.2.1 (2016-09-07)
// Decompilation date: 2017-08-06 19:19:08
