...
// Address range: 0x8f65ba64 - 0x8f65bb33
int32_t othercase(int32_t c, int32_t a2, int32_t a3, int32_t a4) {
    // 0x8f65ba64
    g8 = a2;
    int32_t v1 = g7; // 0x8f65ba64
    char * v2 = g6;
    int32_t v3 = (int32_t)v2;
    if (isalpha(c) == 0) {
        // 0x8f65ba88
        g8 = (int32_t)"ASSERT FAILED at (%s:%d): %s\n";
        _panic(v1, (int32_t)"ASSERT FAILED at (%s:%d): %s\n", (int32_t)"lib/regex/regcomp.c", (int32_t)"isalpha(ch)" & -0x10000 | 1025, (int32_t)"isalpha(ch)", a2, a3);
        // branch -> 0x8f65baa4
    }
    int32_t v4 = isupper(c); // 0x8f65baa8
    int32_t result; // R0
    if (v4 == 0) {
        int32_t v5 = islower(c); // 0x8f65bad8
        result = v5;
        g12 = v3;
        if (v5 == 0) {
            // 0x8f65bb04
